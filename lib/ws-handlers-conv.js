...
export function register(router, deps) {
  ...
  router.handle('conv.steer', async (p) => {
    const conv = queries.getConversation(p.id);
    if (!conv) notFound('Conversation not found');
    if (!p.content) fail(400, 'Missing content');

    // Steer = send a follow-up message, resuming the claude session.
    // claude --print exits after each response, so stdin-based steering is not viable.
    // Instead we queue/run the message exactly like msg.stream does.
    const message = queries.createMessage(p.id, 'user', p.content);
    const agentId = conv.agentId || 'claude-code';
    const isStreaming = activeExecutions.has(p.id);

    if (isStreaming) {
      // Agent is currently running — queue for after it finishes
      if (!messageQueues.has(p.id)) messageQueues.set(p.id, []);
      const queue = messageQueues.get(p.id);
      const seq = getNextQueueSeq(p.id);
      queue.push({ messageId: message.id, content: p.content, agentId, model: conv.model || null, subAgent: conv.subAgent || null, seq });
      broadcastSync({ type: 'queue_status', conversationId: p.id, queueLength: queue.length, seq, timestamp: Date.now() });
    } else {
      // Agent is idle — start immediately (will resume via claudeSessionId)
      broadcastSync({ type: 'message_created', conversationId: p.id, message, timestamp: Date.now() });
      const session = queries.createSession(p.id, agentId, 'pending');
      queries.createEvent('session.created', { messageId: message.id, sessionId: session.id }, p.id, session.id);
      processMessageWithStreaming(p.id, message.id, session.id, p.content, agentId, conv.model || null, conv.subAgent || null)
        .catch(err => console.error('[steer] processMessageWithStreaming error:', err.message));
    }

    return { ok: true, steered: true, conversationId: p.id, messageId: message.id, queued: isStreaming };
  });
