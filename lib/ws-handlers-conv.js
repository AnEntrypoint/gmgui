...
export function register(router, deps) {
  ...
  router.handle('conv.steer', (p) => {
    ...
    console.log(`[steer] conv=${p.id} entry=${!!entry} entry.proc=${!!(entry && entry.proc)} mapProc=${!!activeProcessesByConvId.get(p.id)} stdin=${!!(proc && proc.stdin)} stdinDestroyed=${proc && proc.stdin && proc.stdin.destroyed}`);
    ...
    // No live process — log what we have for diagnosis
    console.log(`[steer] No live process for ${p.id}, falling back to queue/resume`);
