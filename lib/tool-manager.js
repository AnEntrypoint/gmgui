...
const getCliVersion = (pkg) => {
  try {
    ...
    const bin = binMap[pkg];
    if (bin) {
      // Shorter timeout for version checks (some tools hang)
      const out = execSync(`${bin} --version`, { stdio: 'pipe', timeout: 2000, encoding: 'utf8' });
      const match = out.match(/(\d+\.\d+\.\d+)/);
      if (match) {
        console.log(`[tool-manager] CLI ${pkg} (${bin}) version: ${match[1]}`);
        return match[1];
      } else {
        console.log(`[tool-manager] CLI ${pkg} (${bin}) version format not recognized: ${out.trim().substring(0, 50)}`);
        return 'unknown';
      }
    }
  } catch (err) {
    const isTimeout = err.killed || err.message.includes('ETIMEDOUT');
