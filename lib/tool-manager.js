  ...
  try {
    ...
    if (bin) {
      try {
        ...
        // If version detection times out or fails, just mark as installed (we know binary exists)
        return 'installed';
      }
    }
  } catch (err) {
    // If anything fails, mark as installed (we already confirmed binary exists)
    return 'installed';
  }
  return 'installed';
};
